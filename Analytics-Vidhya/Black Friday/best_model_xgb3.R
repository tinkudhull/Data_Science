...
?xgboost
bst = xgboost(data = as.matrix(c.train[,predictors]),
              label = c.train[,outcomename],
              max.depth = 15, nrounds = 50, verbose = 1)
pred = predict(bst, data.matrix((c.test[,-16])))
sub_xgb =data.frame(User_ID = test$User_ID, Product_ID = test$Product_ID,
                    Purchase =  pred)
...
sub_xgb2 =data.frame(User_ID = test$User_ID, Product_ID = test$Product_ID,
                     Purchase =  pred2)
write.csv(sub_xgb2, file = "sub_xgb3.csv", row.names = F)
